...
const generateCertificate = async (req) => {
  ...
  await bezaServiceGateway
    ...
    .then(async (res) => {
      ...
      res.formValue.applicationDate = dateTimeFormattor.getApplicationDate(
      ...
      const commonFieldValue = await bezaServiceGateway.getCommonFileds(
        req.body.investorId
      );
      ...
      const deskUserSignature = await bezaServiceGateway.getdeskUserSignature(
    ...
    .then(async (buffer) => {
      ...
      response = await bezaServiceGateway.uploadAndSave(
        buffer,
        userSopById.id,
        req.body.processInstanceId,
        req.body.isRevoke,

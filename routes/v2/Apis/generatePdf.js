...
const { getFormValueByApplicationID, getCertificateInfo, getCommonFileds, getdeskUserSignature, uploadAndSave } = require('../../../services/gateway_services/bezaServiceGateway');
const { encrypt } = require('../../../util/encryption');
const { config } = require('../../../config/config');
const { generateQR, generateBarcode } = require('../../../services/certificateGeneratorService');
const { AllSopsCodes } = require('../../../shared/constants/AllSopsCodes');
const { getFormatDate, getValidTillDate, getApplicationDate } = require('../../../util/dateTimeFormattor');
...
const generatePdf = async (req, res, next) => {
    try {
        ...
        await getFormValueByApplicationID(req.body.applicationId)
            .then(async (res) => {
                ...
                if (res.isCancellation) {
                    ...
                    const certificateGenerateDate = getFormatDate(new Date(res.parentApprovalDate));
                    ...
                    res.formValue.backgroundImg = background_cancelled;
                    const certificateInfo = await getCertificateInfo(req.body.applicationId)
                    const certificateGenerateDate = getFormatDate(certificateInfo.createdAt)
                    res.formValue.certificateGenerateDate = certificateGenerateDate;
                    res.formValue.validTill = getValidTillDate(certificateGenerateDate);
                    ...
                    const certificateGenerateDate = getFormatDate(new Date(res.approvalDate))
                ...
                res.formValue.applicationDate = getApplicationDate(new Date(res.submittedDate).toLocaleDateString());
                ...
                if (res.sopCode === "ROYALTY_FEE" || res.sopCode === "TECHNICAL_KNOW_HOW_FEE") {
                    res.formValue.lastAmendmentDate = res.formValue.hasOwnProperty('lastAmendmentDate') ?
                        '<div class="content-info"><p class="content-label">Amendment Date:</p><p class="content-value">' + getFormatDate(res.approvalDate) + '</p></div>' : ' ';
                } else {
                    res.formValue.lastAmendmentDate = res.formValue.hasOwnProperty('lastAmendmentDate') ?
                ...
                const deskUserSignature = await getdeskUserSignature(req.body.processInstanceId,"RD_3");
            ...
            .then(async (buffer) => {
                ...
                response = await uploadAndSave(buffer, userSopById.id, req.body.processInstanceId,
            ...
            .catch((err) => {
                logger.err("error happened in generatePdf file, error")
            });
        res.json({ status: 200, msg: 'ejs req get success' })
    } catch (error) {
        next(error);
    }
}
